 ...
 *
 * @param {string} breed - The companion breed identifier
 * @param {string} companionName - The user's companion name
 * @param {string} context - Message context: daily_reminder, streak_risk, streak_lost, welcome_back, milestone
 * @param {number} [streakDays] - Optional streak day count for streak-related messages
 * @returns {{ title: string, body: string }}
 */
function getCompanionMessage(breed, companionName, context, streakDays) {
  const breedMessages = breedTemplates[breed];

  // Fallback to golden_retriever if breed not found
  const templates = breedMessages
...
/**
 * Generic reading reminder message (used when no companion breed is set).
 *
 * @param {string} userName - The user's display name
 * @param {number} duration - Reading duration goal in minutes
 * @returns {{ title: string, body: string }}
 */
function getReadingReminderMessage(userName, duration) {
  const templates = [
    {
      title: "Reading time is almost here!",
      body: `Hey ${userName}! Your ${duration}min reading time starts in 10 minutes. Paige is waiting for you!`,
    },
    {
      title: `${userName}, grab your book!`,
      body: `Your ${duration}-minute reading session begins soon. Find a cozy spot and get ready!`,
    },
    {
      title: "Time to read!",
      body: `Hey ${userName}! ${duration} minutes of reading await. Your next adventure starts in 10 minutes!`,
    },
  ];
...
/**
 * Generic streak risk message (used when no companion breed is set).
 *
 * @param {string} userName - The user's display name
 * @param {number} streakDays - Current streak day count
 * @returns {{ title: string, body: string }}
 */
function getStreakRiskMessage(userName, streakDays) {
  const templates = [
    {
      title: "Your streak is at risk!",
      body: `Your ${streakDays}-day streak is at risk! Just 5 minutes of reading will keep it alive, ${userName}.`,
    },
    {
      title: `${streakDays} days on the line!`,
      body: `Hey ${userName}, don't let your ${streakDays}-day streak slip away! A single page is all it takes.`,
    },
    {
      title: "Don't break the chain!",
      body: `${userName}, your incredible ${streakDays}-day reading streak needs you today. Just 5 minutes!`,
    },
  ];
...
/**
 * Weekly report summary message.
 *
 * @param {string} userName - The user's display name
 * @param {number} xp - XP earned this week
 * @param {number} streak - Current streak day count
 * @param {number} pages - Estimated pages read this week
 * @returns {{ title: string, body: string }}
 */
function getWeeklyReportMessage(userName, xp, streak, pages) {
  const templates = [
    {
      title: "Your Weekly Reading Report",
      body: `This week: ${xp} XP, ${streak} day streak, ~${pages} pages. Amazing work, ${userName}!`,
    },
    {
      title: `Great week, ${userName}!`,
      body: `Weekly recap: ${xp} XP earned, ${streak}-day streak going strong, ~${pages} pages read. Keep it up!`,
    },
    {
      title: "Week in Review",
      body: `${userName}, you crushed it! ${xp} XP, ${streak} day streak, and ~${pages} pages this week. See you next week!`,
    },
  ];
