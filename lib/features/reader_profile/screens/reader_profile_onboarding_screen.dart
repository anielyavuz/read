...
class ReaderProfileOnboardingScreen extends StatefulWidget {
  const ReaderProfileOnboardingScreen({super.key});

  @override
  State<ReaderProfileOnboardingScreen> createState() =>
      _ReaderProfileOnboardingScreenState();
}

class _ReaderProfileOnboardingScreenState
    extends State<ReaderProfileOnboardingScreen> with TickerProviderStateMixin {
          ...
          body: Container(
            decoration: const BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [AppColors.backgroundDark, Color(0xFF1A1B3A)],
              ),
            ),
            child: SafeArea(
  ...
  Widget _buildAppBar(BuildContext context, ReaderProfileState state) {
    return Padding(
          ...
          IconButton(
            ...
            onPressed: () {
              if (state.currentQuestion > 0) {
                context.read<ReaderProfileCubit>().previousQuestion();
  ...
  Widget _buildFinishButton(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: () async {
          await getIt<UserProfileService>().completeOnboarding();
          if (context.mounted) {
            context.go('/home');
          }
        },
  ...
  Widget _buildBottomButton(
      BuildContext context, ReaderProfileState state) {
